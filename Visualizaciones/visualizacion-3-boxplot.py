...
# -*- coding: utf-8 -*-
"""
Created on Tue May 20 01:26:52 2025
...
tabla_bibilioteca = (bibliotecasOriginal[['nro_conabip', 'nombre', 'fecha_fundacion', 'mail', 'id_departamento']]
                     .drop_duplicates()
...
#%% correccion ids poblacion y departamento
...
#%% Visualización 3 - boxplot de cantidad de EE en departamentos por provincia

EEPorDepartamentoPorProvincia = dd.sql("""
                                       SELECT DISTINCT p.nombre AS provincia, 
                                       d.nombre_departamento AS departamento, COUNT(ee.*) AS cantidad_ee
                                       FROM tabla_provincia AS p
                                       INNER JOIN tabla_departamento AS d
                                       ON p.id_provincia = d.id_provincia
                                       INNER JOIN tabla_establecimiento_educativo AS ee
                                       ON d.id_departamento = ee.id_depto
                                       GROUP BY provincia, departamento
                                       ORDER BY provincia, cantidad_ee DESC
                                       """).df()
                                       
EEAGraficar = EEPorDepartamentoPorProvincia
EEAGraficar = EEAGraficar[["provincia","cantidad_ee"]]
medianas = EEAGraficar.groupby("provincia").median().sort_values(by = 'cantidad_ee').rename(columns={'cantidad_ee':'mediana'})
EEAGraficar = EEAGraficar.merge(medianas,on="provincia").sort_values(by = ['mediana']).reset_index()
EEAGraficar = EEAGraficar[["provincia","cantidad_ee"]]
EEAGraficar = EEAGraficar[EEAGraficar["provincia"] != "Ciudad Autónoma de Buenos Aires"]
fueraDelBoxplot = EEPorDepartamentoPorProvincia[EEPorDepartamentoPorProvincia["cantidad_ee"] > 500].sort_values(by=["cantidad_ee"], ascending = False )                                 

EEAGraficar.loc[EEAGraficar['provincia'] == "Santiago del Estero",'provincia'] = "S. del Estero"
EEAGraficar.loc[EEAGraficar['provincia'] == 'Tierra del Fuego','provincia'] = "T. del Fuego"                                   


ax = sns.boxplot(x="provincia",y="cantidad_ee",data=EEAGraficar, color = 'skyblue',gap=0.2,whis=2.05,medianprops={"color": "g", "linewidth": 2})
plt.ylim(0,510)
ax.set_yticks(range(0, 510, 50))
ax.set_title("Cantidad de EE en cada departamento por provincia")
ax.set_ylabel("Total de EE en departamento")
ax.set_xlabel("Provincia")
plt.xticks(rotation=45,ha="right")
plt.tight_layout()
plt.savefig('boxplot_ee_x_provincia.png')

fueraDelBoxplot.to_csv("DeptosFueraDelBoxplot.csv",index=False)
medianas.to_csv("MedianasEEPorProvincia.csv",index =False)